...
import { FileText, Upload, Plus, Eye, Edit, Trash2, Calendar, TrendingUp, Award, Users, CreditCard, Zap, Brain, Target, Mic, Palette } from 'lucide-react';
    ...
    <>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            >
              <FloatingCard delay={0.1}>
                <Card 
                  className="card-3d glow-blue cursor-pointer hover:shadow-lg transition-all duration-200"
              ...
              <FloatingCard delay={0.2}>
                <Card 
                  className="card-3d glow-green cursor-pointer hover:shadow-lg transition-all duration-200"
              ...
              <FloatingCard delay={0.3}>
                <Card 
                  className="card-3d glow-purple cursor-pointer hover:shadow-lg transition-all duration-200"
              ...
              <FloatingCard delay={0.4}>
                <Card 
                  className="card-3d glow-pink cursor-pointer hover:shadow-lg transition-all duration-200"
                  onClick={() => navigate('/jobs')}
                >
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            {/* Cross-Module Quick Actions - PRD Section 19.2 */}
            <Card className="mb-8">
              <CardHeader>
                <CardTitle className="flex items-center">
                ...
                <CardDescription>
                  Jump directly into cross-module workflows
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <Button 
                    variant="outline" 
                    className="h-auto p-4 flex flex-col items-center space-y-2 min-h-[80px]"
                    onClick={() => navigate('/cvs?analyze=quick')}
                  >
                    <Brain className="h-6 w-6 text-blue-600" />
                    <span className="text-sm font-medium">Analyze CV</span>
                    <span className="text-xs text-gray-500">Get AI feedback</span>
                  </Button>
                  
                  <Button 
                    variant="outline" 
                    className="h-auto p-4 flex flex-col items-center space-y-2 min-h-[80px]"
                    onClick={() => navigate('/jobs?cv=optimize')}
                  >
                    <Target className="h-6 w-6 text-green-600" />
                    <span className="text-sm font-medium">Find Jobs</span>
                    <span className="text-xs text-gray-500">Based on your CV</span>
                  </Button>
                  
                  <Button 
                    variant="outline" 
                    className="h-auto p-4 flex flex-col items-center space-y-2 min-h-[80px]"
                    onClick={() => navigate('/interviews?cv=practice')}
                  >
                    <Mic className="h-6 w-6 text-purple-600" />
                    <span className="text-sm font-medium">Practice Interview</span>
                    <span className="text-xs text-gray-500">Using your CV</span>
                  </Button>
                  
                  <Button 
                    variant="outline" 
                    className="h-auto p-4 flex flex-col items-center space-y-2 min-h-[80px]"
                    onClick={() => navigate('/cv-builder?template=job')}
                  >
                    <Palette className="h-6 w-6 text-orange-600" />
                    <span className="text-sm font-medium">Create CV</span>
                    <span className="text-xs text-gray-500">Job-specific template</span>
                  </Button>
                </div>
              </CardContent>
            </Card>
      ...
      {selectedCV && (
        <CVPreviewModal
          ...
          onSaved={loadCVs}
          onAnalyze={async (cvId: string) => {
            // Navigate to CVs page with analysis parameter
            navigate(`/cvs?analyze=${cvId}`);
            setSelectedCV(null);
          }}
          isNewUpload={false}
        />
