...
export const CVs = () => {
  ...
  const handleUpload = async (fileUrl: string) => {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.user) return;
    await supabase.from('uploaded_cvs').insert({
            ...
            <CardContent>
              {uploadedCVs.length === 0 ? (
                ...
                <ul className="space-y-4">
                  {uploadedCVs.map(cv => (
                    <li key={cv.id} className="border p-2 rounded">
                      <a href={cv.file_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View PDF</a>
                      <span className="ml-2 text-xs text-slate-500">Uploaded: {new Date(cv.created_at).toLocaleDateString()}</span>
                    </li>
                  ))}
                </ul>
              )}
