...
export function CreativeDesignerTemplate({ data }: { data: any }) {
  const { personalInfo = {}, experience = [], education = [], skills = [], certifications = [], languages = [], references = [] } = data || {};
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 rounded shadow border max-w-4xl mx-auto font-[Montserrat]">
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="text-sm text-purple-700 mb-2">{personalInfo.email}</div>
          <div className="italic text-gray-600 mb-2">{personalInfo.summary}</div>
        </div>
        <div>
          <div className="font-bold text-purple-700 mb-1">Skills</div>
          <div className="flex flex-wrap gap-1 mb-2">
            {skills.map((skill: string, idx: number) => (
              <span key={idx} className="bg-purple-100 text-purple-800 px-2 py-1 rounded text-xs">{skill}</span>
            ))}
          </div>
        </div>
        <div>
          <div className="font-bold text-purple-700 mb-1">Languages</div>
        ...
        <div>
          <div className="font-bold text-purple-700 mb-1">Certifications</div>
          <ul className="list-disc pl-5 text-xs">
      ...
      <div className="space-y-6">
        <div>
          <div className="font-bold text-purple-700 mb-1">Experience</div>
          <ul className="list-disc pl-5 text-xs">
        ...
        <div>
          <div className="font-bold text-purple-700 mb-1">Education</div>
          <ul className="list-disc pl-5 text-xs">
        ...
        {references.length > 0 && (
          <div>
            <div className="font-bold text-purple-700 mb-1">References</div>
            <ul className="list-disc pl-5 text-xs">
