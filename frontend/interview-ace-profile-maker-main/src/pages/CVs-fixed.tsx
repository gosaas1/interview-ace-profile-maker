...
export const CVs = () => {
  ...
  const generateCVText = (cv: CVData): string => {
    ...
    if (cv.job_title) cvText += `${cv.job_title}\n`;
    if (cv.email) cvText += `Email: ${cv.email}\n`;
    if (cv.phone) cvText += `Phone: ${cv.phone}\n`;
    if (cv.location) cvText += `Location: ${cv.location}\n`;
    if (cv.linkedin_url) cvText += `LinkedIn: ${cv.linkedin_url}\n`;
    if (cv.portfolio_url) cvText += `Portfolio: ${cv.portfolio_url}\n\n`;
            ...
            <div className="mb-8">
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button onClick={() => setShowCVBuilder(true)}>
