...
const OneButtonApply: React.FC<OneButtonApplyProps> = ({ 
  ...
  const tailorCVForJob = async (job: JobPosting, originalCV: CVData): Promise<CVData> => {
    ...
    try {
      ...
      const tailored: CVData = {
        ...originalCV,
        // Keep all personal information unchanged
        full_name: originalCV.full_name,
        email: originalCV.email,
        phone: originalCV.phone,
        location: originalCV.location,
        linkedin_url: originalCV.linkedin_url,
        
        // Keep education unchanged
        education: originalCV.education,
        
        // ENHANCE: Skills optimization for job requirements
        skills: optimizeSkillsForJob(originalCV.skills, job.requirements, job.keywords),
        
        // ENHANCE: Profile summary optimization
        summary: createOptimizedSummary(originalCV, job),
        
        // ENHANCE: Experience descriptions with job-specific achievements
        experiences: originalCV.experiences?.map((exp: any) => ({
          ...exp,
          // Keep job title, company, dates unchanged
          role: exp.role || exp.job_title,
          company: exp.company,
          duration: exp.duration || `${exp.start_date} - ${exp.end_date}`,
          // Enhance description with job-specific achievements and keywords
          description: enhanceExperienceDescription(exp.description, job.requirements, job.keywords, exp.role || exp.job_title)
        })) || [],
        
        // ENHANCE: Add job-specific projects if relevant
        projects: enhanceProjects(originalCV.projects || [], job.requirements, job.keywords),
        
        // ENHANCE: Add relevant certifications (convert to string)
        certifications: typeof originalCV.certifications === 'string' 
          ? originalCV.certifications 
          : enhanceCertifications(originalCV.certifications || [], job.requirements).map(cert => cert.name).join(', '),
        
        // ENHANCE: Add job-specific languages if relevant
        languages: enhanceLanguages(originalCV.languages || [], job.requirements)
      };
  ...
  const getValidExperiences = (tailoredCV: any, userCV: any): any[] => {
    ...
    if (experiences.length === 0 && Array.isArray(userCV?.experiences)) {
      experiences = userCV.experiences.filter(isValidExp);
    }
  ...
  const uniqueSkills = getUniqueSkills(tailoredCV?.skills);
  ...
  const handleEditExperience = (idx: number, field: string, value: string) => {
    setEditCV((prev: any) => {
      ...
      updated.experiences[idx] = { ...updated.experiences[idx], [field]: value };
      return updated;
    });
  };
  ...
  const handleAddExperience = () => {
    setEditCV((prev: any) => ({
      ...prev,
      experiences: [...(prev.experiences || []), { role: '', company: '', duration: '', description: '' }]
    }));
  };
